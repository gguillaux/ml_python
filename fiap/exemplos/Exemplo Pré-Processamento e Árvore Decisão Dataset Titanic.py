...
plot_tree(classificador, filled=True, feature_names =  titanic_dmy.columns.values[1:7], class_names = ['Não Sobreviveu', 'Sobreviveu'])



#Gerando a Arvore de decisão com apenas 3 nós folha
classificador = DecisionTreeClassifier(max_leaf_nodes  = 3)
classificador.fit(X_train, y_train)
y_pred = classificador.predict(X_test)
...
def plot_tree_graphviz():
    ...
    export_graphviz(classificador, out_file=dot_data,  
                    ...
                    feature_names =  titanic_dmy.columns.values[1:7],
...
#Criação da Árvore de decisão com 10 níveis de profundidade
...
# Cria PDF
graph.write_pdf("Árvore Titanic.pdf")

# Cria PNG
graph.write_png("Árvore Titanic.png")


#Criação da Árvore de decisão com 30 níveis de profundidade
classificador = DecisionTreeClassifier(max_depth  = 30)
classificador.fit(X_train, y_train)
y_pred = classificador.predict(X_test)
